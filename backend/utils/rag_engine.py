...
class RAGEngine:
    ...
    def __init__(self, client: OpenAI | None = None, embedding_model: "SentenceTransformer | None" = None):
        ...
        self.use_hash_embeddings = False
        self.hash_embedding_dimension = 384
    ...
    def _embed(self, texts: list[str]) -> list[list[float]]:
        ...
        if self.embed_model is None:
            try:
                ...
                self.embed_model = SentenceTransformer("BAAI/bge-small-en-v1.5", local_files_only=True)
            except Exception as exc:
                logger.warning(
                    "Could not load the SentenceTransformer embedding model from the local cache. "
                    "Falling back to deterministic hash embeddings. Error: %s",
                    exc,
                )
        ...
        try:
            ...
            logger.exception(
                "Could not encode text with the SentenceTransformer embedding model. "
    ...
    def _hash_embed(self, texts: list[str]) -> list[list[float]]:
        ...
        for text in texts:
            vector = [0.0] * self.hash_embedding_dimension
            tokens = re.findall(r"[A-Za-z0-9_]+", text.lower())
            if not tokens:
                tokens = [text]
            for token in tokens:
                digest = hashlib.blake2b(token.encode("utf-8"), digest_size=8).digest()
                bucket = int.from_bytes(digest[:4], "big") % self.hash_embedding_dimension
